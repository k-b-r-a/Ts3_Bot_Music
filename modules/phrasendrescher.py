...
def setup_quoter(ts3bot, db):
    ...
    curs.execute(
        'CREATE UNIQUE INDEX IF NOT EXISTS Quotesididx ON Quotes (id)')
    conn.commit()
    conn.close()
...
def add_quote(sender, msg):
    if len(msg) <= len('!quote '):
        Bot.send_msg_to_client(bot.ts3conn, sender,
                               'Please include a quote to save.')
    else:
        conn = sqlite3.connect(path)
        c = conn.cursor()
        ...
        c.execute(
            'INSERT INTO Quotes (quote, submitter, time, shown) VALUES (?, ?, strftime("%s", "now"), ?)', (quote, submitter, 0))
        conn.commit()
        conn.close()
        Bot.send_msg_to_client(bot.ts3conn, sender,
